  ...
  var app = {
    ...
    handleBackButton: function() {
        ...
        if (successdiv.style.display == "block") {
            _czc.push(['_trackEvent', TVmodel,'领取结果页','返回', '','getimmediateDiv']);
            successdiv.style.display = "none";
            showgoldvip();
        }
        //首页点击返回----1.活动首页，返回        
        else if(goldvip.style.display == "block"){
            _czc.push(['_trackEvent', TVmodel, '黄金VIP页', '返回','','getimmediateDiv']);
           experience();
        }
            ...
            _czc.push(['_trackEvent', TVmodel, '活动首页', '返回','','getimmediateDiv']);
            experience();
...
function experience(){
    ...
    if(time <=0 || time >300){time = 60};
    _czc.push(['_trackEvent', TVmodel, '页面停留时长（秒）','',time,'getimmediateDiv']);
    navigator.app.exitApp();
}

function experienceonclick(){
    if (buttonFlag == 0) {
        _czc.push(['_trackEvent', TVmodel, '活动首页', '立即领取点击次数','','getimmediateDiv']);
        buttonFlag = 1;
    };
...
function sendHTTP1() {
 document.getElementById('loading').style.display="block";
    ...
    $.ajax({             
         ...
         error: function(){ 
            console.log("error"); 
            _czc.push(['_trackEvent', TVmodel, '活动首页', '点击异常:未获取到后台数据','','']);
        } 
    });
}
