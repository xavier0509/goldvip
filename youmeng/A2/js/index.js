  ...
  var app = {
    ...
    handleBackButton: function() {
        ...
        if (successdiv.style.display == "block") {
            _czc.push(['_trackEvent', '2.爱奇艺领取页','返回', '', '','getimmediateDiv']);
            successdiv.style.display = "none";
            showgoldvip();
        }
        //首页点击返回----1.活动首页，返回        
        else if(goldvip.style.display == "block"){
            _czc.push(['_trackEvent', '3.领取黄金会员扫码页面', '返回', '','','getimmediateDiv']);
            time2 = new Date().getTime();
            time = (time2 - time1)/1000;
            _czc.push(['_trackEvent', '受访页面', '持续时长', '秒',time,'getimmediateDiv']);
            navigator.app.exitApp();
        }
        else{
            _czc.push(['_trackEvent', '1.活动首页', '返回', '','','getimmediateDiv']);
            time2 = new Date().getTime();
            time = (time2 - time1)/1000;
            _czc.push(['_trackEvent', '受访页面', '持续时长', '秒',time,'getimmediateDiv']);
            navigator.app.exitApp();
    ...
    onDeviceReady: function() {
        ...
        _czc.push(['_trackEvent', '1.活动首页', '弹出', '','','getimmediateDiv']);
        ...
        coocaaosapi.hasCoocaaUserLogin(function(message) {
            ...
            if (loginstatus == "false") {
                _czc.push(['_trackEvent', '登录情况', '未登录', '','','getimmediateDiv']);
                document.getElementById('getimmediate').src=app.rel_html_imgpath(__uri("../images/noload.png"));
            }
            else{    
                _czc.push(['_trackEvent', '登录情况', '已登录', '','','getimmediateDiv']);
                }
...
function experienceonclick(){
    _czc.push(['_trackEvent', '1.活动首页', '点击button', '立即领取点击次数','','getimmediateDiv']);
    ...
    if(loginstatus=="false"){//页面加载时统计登录情况
        ...
        coocaaosapi.addUserChanggedListener(function(message){
            console.log(message);
            document.getElementById('getimmediate').src=app.rel_html_imgpath(__uri("../images/getnow.png"));
            loginstatus = "true";
        });
    }
...
function getsuccess(){
    ...
    document.getElementById("getimmediateDiv").addEventListener("click", experience);
}
...
function sendHTTP1() {
 document.getElementById('loading').style.display="block";
    ...
    $.ajax({             
         ...
         error: function(){ 
            console.log("error"); 
            _czc.push(['_trackEvent', '1.活动首页', '点击异常情况', '未获取到后台数据','','']);
        } 
    });
}
...
function receive(data) {
    ...
    {
        _czc.push(['_trackEvent', '2.爱奇艺领取页','影视会员领取情况', '领取成功', '','getimmediateDiv']);
    ...
    {
    	//抱歉，您已经领取过了
        _czc.push(['_trackEvent', '2.爱奇艺领取页','影视会员领取情况', '领取失败', '','getimmediateDiv']);
        _czc.push(['_trackEvent', '2.爱奇艺领取页','影视会员领取失败原因', '已经领取过', '','getimmediateDiv']);
        console.log(data.result.code);
        document.getElementById('loading').style.display="none";
        document.getElementById("getimmediate").src=app.rel_html_imgpath(__uri("../images/nochance.png"));
    ...
    else 
    {
    	...
    	//异常，请稍后再试
        _czc.push(['_trackEvent', '2.爱奇艺领取页','影视会员领取情况', '领取失败', '','getimmediateDiv']);
        _czc.push(['_trackEvent', '2.爱奇艺领取页','影视会员领取失败原因', '数据异常', '','getimmediateDiv']);
        document.getElementById('loading').style.display="none";
...
function showgoldvip(){
    if (vipFlag == 0) {
    _czc.push(['_trackEvent', '2.爱奇艺领取页','点击button', '领取黄金会员', '','getimmediateDiv']);};
    ...
    $.ajax({             
         ...
         error: function(){ 
            _czc.push(['_trackEvent', '2.爱奇艺领取页','点击异常情况', '', '','getimmediateDiv']);
            console.log("error"); 
        } 
    });
    // generateQRCode("测试");
...
function receive2(data) {
    if (vipFlag == 0) {
    _czc.push(['_trackEvent', '3.领取黄金会员扫码页面','弹出次数', '', '','getimmediateDiv']);};
    document.getElementById('loading2').style.display="none";
    console.log("receive:" + data);
    console.log("code:" + data[0].code);    
    if(data[0].code=="0000")//成功
    {
        _czc.push(['_trackEvent', '3.领取黄金会员扫码页面','二维码显示', '成功', '','getimmediateDiv']);
    ...
    {
        if (vipFlag == 0) {
        _czc.push(['_trackEvent', '3.领取黄金会员扫码页面','二维码显示', '失败', '','getimmediateDiv']);};
        vipFlag = 1;
